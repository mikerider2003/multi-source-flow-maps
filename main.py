...
def main_clustered(n_clusters=None, show_intra=None, multiple_bundle_points=True, bundle_radius=3.0, split_radius=1.5, print_table=False):
    """
    ...
    for i, m in enumerate(sorted(clusters.keys())):
        ...
        if multiple_bundle_points:
            mmb2(gdf, filtered, centroid_table, clusters, show_intra=show_intra, ax=ax, bundle_radius=bundle_radius, split_radius=split_radius, 
                 estimated_exports=estimated_exports, crossing_scores=crossing_scores, distance_scores=distance_scores, bundling_scores=bundling_scores)
        else:
